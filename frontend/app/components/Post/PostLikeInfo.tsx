...
type PostLikeInfoPropsType = {
  id: number;
};

export const PostLikeInfo: React.FC<PostLikeInfoPropsType> = ({ id }) => {
  const Post_Liked_Url = process.env.NEXT_PUBLIC_BASE_URL + "posts/" + id + "/liked";
  ...
  const { data, error } = useSWRImmutable(isShow ? Post_Liked_Url : null);
