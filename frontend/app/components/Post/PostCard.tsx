...
export const PostCard: React.FC<PostCardProps> = ({ post, name, gravator_url }) => {
  ...
  //Post送信先用のUrl
  const Post_Edit_Url = process.env.NEXT_PUBLIC_BASE_URL + "posts/" + post.id;
      ...
      <>
        {user_data?.user?.id === post.user_id ? (
          <Row>
            ...
            <Col xs={3} md={3}>
              <PostEdit isEdit={isEdit} setIsEdit={setIsEdit} />
            </Col>
            <Col xs={2} md={3}>
